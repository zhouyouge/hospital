            ...
            <form class="layui-form" action="">
                ...
                <div class="layui-form-item">
                    ...
                    <input type="text" name="username" lay-verify="required|account" placeholder="用户名或者邮箱"
                           autocomplete="off" id="username" class="layui-input" value="">
                </div>
                <div class="layui-form-item">
                    <label class="layui-icon layui-icon-password" for="password"></label>
                    <input type="password" name="password" lay-verify="required|password" placeholder="密码"
                           autocomplete="off" class="layui-input" id="password" value="">
                </div>
                <div class="layui-form-item">
                    <label class="layui-icon layui-icon-vercode" for="captcha"></label>
                    <input type="text" name="captcha" lay-verify="required|captcha" placeholder="图形验证码"
                           autocomplete="off" class="layui-input verification captcha" id="captcha" value="xszg">
                ...
                success: function (data) {
                    if (data.code == 200) {
                        layer.msg('登录成功', function () {
                            if(data.data.departmentsId ==1){//药学部
                                window.location = 'drug.jsp';
                            }else if(data.data.departmentsId ==2){ //门诊医生部
                                window.location = 'cure.jsp';
                            }else if(data.data.departmentsId ==3){//行政管理部
                                window.location = 'stuff.jsp';
                            }
